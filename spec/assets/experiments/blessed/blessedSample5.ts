...
var layout = blessed.layout({
  ...
  renderer(coords: { xl: number; xi: number; yl: number; yi: number; }) {
    ...
    return function iterator(el: { shrink: boolean; position: { left: number; top: number; }; width: number; height: number; }, i: number | undefined) {
      ...
      if (!last) {
        ...
        if (el.position.left + el.width <= width) {
          ...
          // row).
          rowOffset += self.children.slice(rowIndex, i).reduce(function(out: number, el: { lpos: { yl: number; yi: number; }; }) {
